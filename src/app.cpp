...
namespace {
bool matches_option(const QString& givenoption, const QString& expectedoption,
...
App::App(int& argc, char** argv)
  ...
  rootlogger->addAppender(
      new log4cxx::ConsoleAppender(new log4cxx::PatternLayout("[%-5p] %m%n")));
  ...
  while (idx < argc) {
    ...
    if (matches_option(arg, "help", 0) || matches_option(arg, "h") ||
      ...
      if ((idx + 1) >= argc) {
        LOG4CXX_FATAL(_logger, "Option \"" << arg
      ...
      if (eqidx != std::string::npos) {
        ...
        setLogLevel("", param);
               ...
               matches_option(arg, "application-identifier")) {
      ...
      if (_gui) {
        LOG4CXX_WARN(_logger, "Option \"" << arg
...
App* App::INSTANCE() { return _instance; }
...
void App::interactiveMain() { std::cout << "Hello world!" << std::endl; }

void App::consoleMain() { std::cout << "Hello world!" << std::endl; }
...
void App::printHelpMessage() {
  ...
  std::cout << "    --version-triplet            Prints the undecorated "
               "program version."
            << std::endl;
  std::cout << "    --appid                      Prints the unique application "
               "identifier."
            << std::endl;
  ...
  std::cout
      ...
      << std::endl;
  std::cout << "    --loglevel <logger>=<level>  Sets the logging level for "
               "the given logger."
            << std::endl;
...
void App::printVersionMessage() {
  ...
  std::cout << getProjectVendorName() << "; Copyright (C) "
            << getProjectCopyrightYears();
}
...
QString App::getProjectName() { return APPLICATION_NAME; }

QString App::getProjectCodeName() { return APPLICATION_CODENAME; }

QString App::getProjectVendorID() { return APPLICATION_VENDOR_ID; }

QString App::getProjectVendorName() { return APPLICATION_VENDOR_NAME; }

QString App::getProjectID() { return APPLICATION_ID; }
...
QString App::getProjectVersion() { return APPLICATION_VERSION_STRING; }

QString App::getProjectCopyrightYears() { return APPLICATION_COPYRIGHT_YEARS; }

QString App::getProjectInvocation() { return _invocation; }
...
void App::unsetPreference(const std::string& key) {
  ...
  if ((settings.childGroups().length() != 0) ||
      (settings.childKeys().length() != 0)) {
    settings.setValue("", "");
  } else {
    settings.remove("");
  }
...
void App::setLogLevel(const std::string& logger, const std::string& level) {
  log4cxx::LoggerPtr loggerptr =
      ((logger == "") ? (log4cxx::Logger::getRootLogger())
                      : (log4cxx::Logger::getLogger(logger)));
...
log4cxx::LoggerPtr App::_logger = log4cxx::Logger::getLogger("App");
